export default {
  ...
  // src\app.ts
  app: {
    welcome: "Welcome to MCSManager Daemon",
    instanceLoad: "All {{n}} instances are loaded",
    instanceLoadError: "Failed to load local instances configuration:",
    ...
    addr: "Access URL：http://<IP地址>:{{port}}/ or ws://<IP地址>:{{port}}",
    configPathTip: "Configuration files in：data/Config/global.json",
    password: "Access key：{{key}}",
    passwordTip: "Access key is the only way of authentication, go to Nodes section on MCSManager Web to connect",
    exitTip: "You can press Ctrl+C to stop the daemon"
  },
  ...
  common: {
    ...
    killProcess: "Process {{pid}} is force stopped",
    uuidIrregular: "UUID {{uuid}} is illegal"
  },
  ...
  command: {
    ...
    instanceNotOpen: "NO existing process of the instance, command execution failed"
  },
  ...
  instance: {
    ...
    invalidContainerName: "Illegal container name {{v}}",
    successful: "Instance {{v}} is started successfully"
  },
  // src\entity\commands\start.ts
  start: {
    instanceNotDown: "This instance is already running",
    instanceMaturity: "Cannot start this instance due to expiration",
    startInstance: "Preparing to start instances..."
  },
  // src\entity\commands\general\general_restart.ts
  restart: {
    start: "Executing scheduled instance restart",
    error1: "重启实例状态错误，实例已被启动过，上次状态的重启计划取消",
    error2: "重启实例状态错误，实例状态应该为停止中状态，现在变为正在运行，重启计划取消",
    restarting: "Instance stopped, restarting..."
  },
  // src\entity\commands\general\general_start.ts
  general_start: {
    instanceConfigErr: "Command, Code page or working directory is not set",
    cwdPathNotExist: "Working directory does not exist",
    cmdEmpty: "Command not set, cannot start this instance",
    startInstance: "Session {{source}}: Request of starting the instance.",
    instanceUuid: "UUID: [{{uuid}}]",
    startCmd: "Command: {{cmdList}}",
    cwd: "Working directory: {{cwd}}",
    pidErr: `Instance process/container start failed（NO PID）, possible cause is:
    1. Configuration is wrong, go to instance settings and check again
    2. Runtime or Environment is not properly configured. (eg. Java)
    
    Native start command：
    {{startCommand}}
    
    Executable & Parameters:
    Executable：{{commandExeFile}}
    Parameters：{{commandParameters}}
    
    Please contact your IT admin, `,
    startErr: "Failed to start instance, please check start command, environment, config, etc.",
    startSuccess: "Instance {{instanceUuid}} started successfully PID: {{pid}}",
    startOrdinaryTerminal:
      "Instance is running, you can type in command inside the command box. If you want to use advanced feature of the terminal like Ctrl, Tab keys enable the terminal emulation feature in instance settings"
  },
  ...
  // src\service\ui.ts
  ui: {
    help: '[TERMINAL] You can type essential commands to Daemon, for more information, type "help"'
  },
